...
export function ActivitiesSection({ dict }: ActivitiesSectionProps) {
  ...
  return (
    <section id="activities" className="py-20 sm:py-28 bg-gradient-to-b from-lime-500 to-lime-600 text-white">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-4">{dict.activities.title}</h2>
          <p className="text-xl text-lime-100 max-w-2xl mx-auto mb-6">{dict.activities.subtitle}</p>
          <p className="text-lime-100/80 max-w-3xl mx-auto">{dict.activities.intro}</p>
        </div>
            ...
            return (
              <Card key={index} className="border-0 bg-white/10 backdrop-blur-sm hover:bg-white/20 transition-colors">
                <CardContent className="p-6 text-center">
                  ...
                  <p className="text-sm text-lime-100">{activity.desc}</p>
                </CardContent>
              </Card>
            )
          })}
